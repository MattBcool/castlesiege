...
public class MovingFlag extends Flag
{
	public HashMap<Entity, Location> stands;
	private int t1 = 0;
	private int t2 = 0;

	public MovingFlag(Main plugin, ObjectiveData objectiveData)
	{
	...
	public void doIdleAnimation()
	{
		...
		for(Entity stand : stands.keySet())
		{
			...
			double offset2 = (Math.cos(((double) val / 25d) * Math.PI)) * ((double) (id) / 10d);
			double offsetFirst = (Math.cos(((double) val2 / 25d) * Math.PI)) * ((double) (id + 1) / 10d);

			double offset1 = (id * 0.62) - (Math.abs(offsetFirst * 0.25d));
			// offset1-=((double)row*10d) * 0.2;
			double angle = base.getYaw();
			double x = lengthdir_x(offset1, angle) - lengthdir_y(offset2, angle);
			double z = lengthdir_y(offset1, angle) + lengthdir_x(offset2, angle);

			Location loc = stands.get(stand).clone().add(x, (row * 0.62d) + capped / 13d, z);
